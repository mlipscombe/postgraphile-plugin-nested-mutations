...
const kitchenSinkData = () => readFile(`${__dirname}/../data.sql`, 'utf8');

/* eslint-disable global-require, no-return-await */
beforeAll(() => {
  ...
  const queryResultsPromise = (async () => {
    ...
    return await withPgClient(async (pgClient) => {
      ...
      return await Promise.all(queryFileNames.map(async (fileName) => {
        ...
        // differently.
        const schemas = {
          // 'reverse.graphql': gqlSchemas.dynamicJson,
        };
  ...
  queryResults = queryFileNames.map(async (_, i) => await (await queryResultsPromise)[i]);
});
